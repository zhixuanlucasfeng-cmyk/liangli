...
async function testInstallUsesV13ShellCacheWithAccountSyncModule() {
  const harness = createHarness();
  await dispatch(harness, 'install');
  assert.deepEqual(harness.opened, ['liangli-v13']);
  assert.ok(
...
(async () => {
  ...
  await testInstallUsesV13ShellCacheWithAccountSyncModule();
