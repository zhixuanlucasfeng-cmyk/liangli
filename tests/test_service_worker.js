...
async function testInstallUsesV10ShellCacheWithAccountSyncModule() {
  const harness = createHarness();
  await dispatch(harness, 'install');
  assert.deepEqual(harness.opened, ['liangli-v10']);
  assert.ok(
...
(async () => {
  ...
  await testInstallUsesV10ShellCacheWithAccountSyncModule();
