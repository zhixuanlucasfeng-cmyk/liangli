...
const ids=['accountEmail','accountPassword','accountPasswordConfirm','accountPasswordConfirmField','accountSignIn','accountSignUp','accountRecover','accountSignOut','accountSyncNow','accountChangePassword','copyLocalFlashcards','accountFirstLoginChoices','accountPasswordReset','accountUpdatePassword','coreRecoveryItems','accountAuthForm','accountSignedInCard','accountSignedInEmail','accountAuthModeSignIn','accountAuthModeSignUp'];
const elements=new Map(ids.map(id=>[id,{disabled:false,hidden:false,textContent:'',innerHTML:'',type:'password',value:'',setAttribute(name,value){this[name]=value;}}]));
...
assert.equal(elements.get('accountAuthForm').hidden,true,'a signed-in account never sees the registration form');
assert.equal(elements.get('accountSignedInCard').hidden,false,'a signed-in account gets a dedicated confirmation card');
assert.equal(elements.get('accountSignedInEmail').textContent,'owner@example.test','the confirmation card identifies the active account');
context.AccountClient.session=null;context.accountStatus.renderAccountPanel();
assert.equal(elements.get('accountAuthForm').hidden,false,'a signed-out visitor sees the authentication form');
assert.equal(elements.get('accountSignedInCard').hidden,true,'the signed-in card is hidden after sign-out');
context.accountStatus.setAccountAuthMode('signup');
assert.equal(elements.get('accountPasswordConfirmField').hidden,false,'sign-up requires password confirmation');
assert.equal(elements.get('accountSignUp').hidden,false,'sign-up mode exposes one primary account-creation action');
assert.equal(elements.get('accountSignIn').hidden,true,'sign-up mode hides the competing sign-in submit action');
context.accountStatus.toggleAccountPasswordVisibility(true);
assert.equal(elements.get('accountPassword').type,'text');
assert.equal(elements.get('accountPasswordConfirm').type,'text');
(async()=>{
  context.AccountClient.signUpCalls=0;
  context.AccountClient.signUp=async function(email,password){this.signUpCalls++;this.session={user:{id:'u2',email}};return {session:this.session,user:this.session.user};};
  elements.get('accountEmail').value='new@example.test';elements.get('accountPassword').value='secret-one';elements.get('accountPasswordConfirm').value='different';
  await context.accountStatus.signUpAccount();
  assert.equal(context.AccountClient.signUpCalls,0,'mismatched confirmation never submits a new account');
  assert.equal(status.textContent,'authPasswordsMismatch','the visitor gets an immediate password mismatch explanation');
  elements.get('accountPassword').value='secret-one';elements.get('accountPasswordConfirm').value='secret-one';
  await context.accountStatus.signUpAccount();
...
const start = script.indexOf('let accountReturnFocus=');
const end = script.indexOf('async function beginAccountFirstLogin(', start);
const ids=['accountEmail','accountPassword','accountPasswordConfirm','accountPasswordConfirmField','accountSignIn','accountSignUp','accountRecover','accountSignOut','accountSyncNow','accountChangePassword','copyLocalFlashcards','accountFirstLoginChoices','accountPasswordReset','accountUpdatePassword','coreRecoveryItems','accountSyncStatus','accountAuthForm','accountSignedInCard','accountSignedInEmail','accountAuthModeSignIn','accountAuthModeSignUp'];
const elements=new Map(ids.map(id=>[id,{disabled:false,hidden:false,textContent:'',innerHTML:'',type:'password',setAttribute(name,value){this[name]=value;}}]));
const document={body:{style:{}},activeElement:null,querySelector(){return {inert:false,setAttribute(){},removeAttribute(){}};},getElementById:id=>elements.get(id)};
