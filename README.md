...
钱包按你选择的天/周/月/年周期，把「周期总额 − 建议储蓄 + 上期结转」以分为单位尽量平均分到每天。建议储蓄只是给自己预留的参考金额，不会转账、锁定资金或做任何理财决定。当天可用额的计算是 `当天基础额 + 前一天结转 − 当天消费`；所以今天多花的 ¥30 会作为 `−¥30` 平静地带到明天，少花的余额也同样带过去。编辑或删除消费会重新计算这个周期的余额。

营养、钱包和记录数据保存在这台设备浏览器的本地存储；它们不会上传到 Supabase，也不会随着 Flashcards 登录而同步。生活页的「导出生活数据」会下载一份 JSON（仅含热量目标、饮食记录/收藏食物、预算周期和消费记录）；导入前会先验证文件并显示数量摘要，确认后才会替换这四类生活数据。JSON 备份不包含任务、账号或卡片，导入也不会改变它们。清理浏览器数据或更换设备前，请先导出备份。
